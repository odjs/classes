...
function processArray(state: ClassesState, classNames: ClassArray): ClassesState {
  return classNames.reduce(processClassItem, state)
}

function stateToArray(state: ClassesState): string[] {
  const classNames = Object.keys(state)
  return classNames.filter((className) => state[className])
}

function stringifyState(state: ClassesState): string {
  const filteredClassNames = stateToArray(state)
  return filteredClassNames.join(' ')
}

export function classes(...classNames: ClassArray): string {
  const state = processArray({}, classNames)
