...
describe('passing functions to classesToArray function', () => {
  //

  test('Should return classname from resolved classname', () => {
    const expected = 'class1 class2'
    expect(classesToArrayToString(() => expected)).toBe(expected)
    expect(classesToArrayToString(() => () => () => expected)).toBe(expected)
  })
  ...
  test('Should return classname from multiple resolved classnames', () => {
    ...
    expect(classesToArrayToString(resolveClassName1, resolveClassName2)).toBe('class1 class2')
  })
  ...
  test('Should return classname from multiple classnames', () => {
    ...
    expect(classesToArrayToString(resolveArrayClassName)).toBe('class1 class2 class3')
  })
  ...
  test('Should return classname from multiple classnames', () => {
    ...
    expect(classesToArrayToString(resolveClassName)).toBe('class1 class3 class4')
  })
  ...
  test('Should receive current classnames', () => {
    ...
    expect(classesToArrayToString('class1 class4', removeClass1and2, addClass3and4)).toBe('class3 class4')
